  ...
  return (<>
    <Dialog open={props.open} onClose={props.close} maxWidth={false}>
      <div className="bg-white rounded-12 text-center">
        <div className="text-18 font-bold mt-8 mb-4">
          选择头像
        </div>
        <div className="img-box overflow-y-auto py-3 px-8 mb-8">
          <div className="img-grid-box grid gap-x-2 gap-y-2">
            {IMAGES.map((url: string) => (
              <Tooltip
                classes={{ tooltip: 'p-0 bg-white shadow-6 rounded-lg overflow-hidden' }}
                placement="top"
                title={<div className="p-2"><img className="w-40 h-40" src={url} alt="" /></div>}
                key={url}
              >
                <div
                  className={classNames(
                    'group w-20 h-20 p-1 rounded overflow-hidden cursor-pointer relative',
                    'hover:shadow-3 hover:z-10',
                  )}
...
const IMAGES = Array(54).fill(0).map((_, i) => `${basePath}/assets/avatar/${i + 1}.png`)
